...
codes_inds_1 = torch.combinations(torch.arange(K),2,with_replacement=False).unsqueeze(0).narrow(2,1,1).repeat(batch_size//no_parts,1,N)
codes_inds_2 = torch.combinations(torch.arange(K),2,with_replacement=False).unsqueeze(0).narrow(2,0,1).repeat(batch_size//no_parts,1,N)
roll_inds = torch.zeros((N,2*N), dtype=torch.bool)
...
sigmas = torch.triu(torch.ones((N,N),dtype=torch.bool)).unsqueeze(0)

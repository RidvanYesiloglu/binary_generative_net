...
import math
class Param_Info():
    def __init__(self, name, desc, typ, poss, defa, req, ask):
        self.name = name
        self.desc = desc
        self.typ = typ
    ...
    def get_input_and_update_runsets(self, cart_prod_runsets, vals_list, params_dict):
        ...
        if len(cart_prod_runsets) > 0:
            new_prod = []
            for runset_wo_new in list(cart_prod_runsets):
                is_allowing = (self.ask[0] != '0') and ((self.ask[0] == '1') or \
                                            eval('(runset_wo_new[names_list.index(self.ask[1])] {})'.format(self.ask[2])))
                if is_allowing:
                    mylist = list(itertools.product([runset_wo_new], inp_list if inp_list is not None else [None]))
                else:
                    mylist =list(itertools.product([runset_wo_new], [None]))
...
def get_arguments(params_dict, opts_str=None):
    ...
    for param_info in params_dict.param_infos:
        parser.add_argument("--"+param_info.name, help=param_info.desc, type=eval(param_info.typ), choices=param_info.poss, \
                            default=param_info.defa, required=(param_info.req[0] == '1') )
    if opts_str is None:
